...
class CallBloc extends Bloc<CallEvent, CallState> with WidgetsBindingObserver implements CallkeepDelegate {
  ...
  late final ConferenceBridge _conferenceBridge;
  ...
  CallBloc({
    ...
    _conferenceBridge = ConferenceBridge(
      factory: peerConnectionManager.factory,
      userMediaBuilder: userMediaBuilder,
      // The bridge outlives close(): it is torn down after the bloc has
  ...
  Future<void> close() async {
    ...
    await _conferenceBridge.teardown();
  }
  ...
  Future<void> _setRoomMuted(bool muted, Emitter<CallState> emit) async {
    if (!state.conference.isPresent) return;
    await _conferenceBridge.setSelfMuted(muted);
  ...
  Future<void> __onMutationConferenceOffer(_CallMutationEventConferenceOffer e, Emitter<CallState> emit) async {
    ...
    try {
      final answer = await _conferenceBridge.answer(room: e.room, offer: e.jsep);
      await _signalingModule.execute(
  ...
  Future<void> __onMutationConferenceRemoteCandidate(
    ...
    if (!state.conference.isPresent) return;
    try {
      await _conferenceBridge.addRemoteCandidate(e.candidate);
    } catch (error, stackTrace) {
  ...
  Future<void> __onMutationConferenceLocalCandidate(
    ...
    if (!state.conference.isPresent || !_conferenceBridge.isUp) return;
    await _executeConferenceRequest(
  ...
  /// Drops the room: the bridge, the OS grouping and the state. With
