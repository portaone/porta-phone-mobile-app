...
class ConferencePanel extends StatelessWidget {
  ...
  /// Ends the room and every call in it.
        ...
        Padding(
          ...
          child: Row(
            children: [
              ...
              _EndButton(onPressed: onEndPressed, buttonStyle: actionsStyle?.hangup),
            ],
          ),
        ),
        _PanelRow(
...
/// Ends the room and, with it, every call in it - which is why it says so
/// rather than carrying a bare glyph.
class _EndButton extends StatelessWidget {
  const _EndButton({required this.onPressed, required this.buttonStyle});
  ...
  Widget build(BuildContext context) {
    return CallActionButton(
      label: context.l10n.call_SemanticsLabel_conferenceEnd,
      identifier: conferenceEndId,
      onPressed: onPressed,
      style: (buttonStyle ?? _fallbackHangupStyle(context)).copyWith(
        padding: const WidgetStatePropertyAll(EdgeInsets.symmetric(horizontal: 16)),
        shape: const WidgetStatePropertyAll(StadiumBorder()),
      ),
      // The word is what the control looks like; its spoken name is above.
      child: ExcludeSemantics(child: Text(context.l10n.call_ConferencePanel_end)),
    );
  }
}
