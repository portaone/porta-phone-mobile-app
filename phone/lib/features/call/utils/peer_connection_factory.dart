...
Map<String, dynamic> rtcConfigurationFrom(
  ...
  return <String, dynamic>{
    ...
    // configuration, so a stated policy has to be written on each entry it
    // applies to - and only those. An entry with no `turns:` URL has no TLS to
    // govern, and stamping it would put `insecure_no_check` where it decides
    // nothing, which reads in a log like a connection that gave up more than
    // it did.
    'iceServers': policy == null ? servers : servers.map((server) => _withPolicy(server, policy)).toList(),
...
/// The entry, with [policy] written on it when it has a `turns:` URL to apply
/// it to, and untouched otherwise.
Map<String, dynamic> _withPolicy(Map<String, dynamic> server, String policy) {
  final urls = server['urls'];
  final carriesTls = switch (urls) {
    String url => url.startsWith('turns:'),
    List<dynamic> list => list.any((url) => url is String && url.startsWith('turns:')),
    // The legacy singular spelling, which the plugin still accepts.
    _ => server['url'] is String && (server['url'] as String).startsWith('turns:'),
  };

  return carriesTls ? <String, dynamic>{...server, 'tlsCertPolicy': policy} : server;
}
