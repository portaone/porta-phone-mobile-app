...
abstract class CdrsListCubit extends Cubit<CdrsListState> {
  ...
  /// the same budget would let it burn this many requests seven times over,
  /// once for each slice the horizon allows.
    ...
    for (final window in historyWindows.backFrom(resumeAt)) {
      ...
      while (true) {
        ...
        if (pagesAsked >= _pagesPerWalkLimit) {
          logger.warning('Gave up after $pagesAsked pages; the backend may be ignoring the page number');
          return;
        }
        page++;
      }
