  ...
  Widget build(BuildContext context) {
    ...
    return BlocBuilder<MediaSettingsCubit, MediaSettingsState>(
      ...
      builder: (context, state) {
        final selected = state.iceSettings.certificateVerification ?? TurnCertificateVerification.verify;
        ...
        return Column(
          ...
          children: [
            ChoosableSection<TurnCertificateVerification>(
              title: null,
              buildOptionTitle: (option) => Text(
                option == TurnCertificateVerification.disabled
                    ? context.l10n.settings_certificateVerification_disabled
                    // `ChoosableSection` renders the unlisted option as null,
                    // which is where verifying lives - the same shape the ICE
                    // filters use for "no filtering".
                    : context.l10n.settings_certificateVerification_verify,
              ),
              options: const [TurnCertificateVerification.disabled],
              selected: selected == TurnCertificateVerification.verify ? null : selected,
              onSelect: (option) => cubit.setCertificateVerification(option ?? TurnCertificateVerification.verify),
            ),
