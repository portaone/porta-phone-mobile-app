...
abstract interface class DestinationPickPurpose {
  ...
  /// Whether taking a choice ends the request, or whether the feature's own
  /// state decides when it is over.
  ///
  /// False where the request mirrors a mode the feature keeps elsewhere. A
  /// transfer is under way until the call says otherwise: a switch that
  /// refuses the REFER leaves the call still looking for a target, and lists
  /// that quietly went back to normal would leave the person with no way to
  /// choose again.
  bool get closedByChoice;
