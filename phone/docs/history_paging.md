...
| `cdr_sync_cursors` | `packages/data/app_database` (one row per kind) | how far back the store has been asked, surviving screens and launches - the `historyWalk` row, beside the `sync` cursor |
...
| the list stops with records still on the server | check `historyEndReached` against the horizon, and the watermark: the `historyWalk` row of `cdr_sync_cursors` may already be at the floor |
