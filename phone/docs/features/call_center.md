...
The **settings row** asks live: the session builds the repository wherever the
capability is on, reads once at its start, and the row reads the answer from
the cubit. An empty list is not an error - it is the backend saying "not an
agent" - and the row is absent for them, exactly as it is where the capability
is off.
