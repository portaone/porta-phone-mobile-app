...
Which sentence goes with which answer is the feature's rule, not a row's, so
it lives in one place - `utils/voicemail_action_reporter.dart`. The screen hands
it an action and gets on with drawing the list; the row that started it is
usually gone by the time there is anything to say.
...
Two 4xx answers are deliberately not "gone": a route this deployment does not
have (these calls are declared optional, so that is a 404 of its own) and a
mailbox that was never configured. Both are about the backend's shape rather
than about this message. The classifier is
`extensions/request_failure.dart` -> `isVoicemailGone`.
