...
exists to remove - and two of them did forget. The same function closes the
request once the choice is taken, for the same reason: by then the feature is
waiting on a backend somewhere, and a request left standing would hold every
list in picking mode with nothing left to pick for.
