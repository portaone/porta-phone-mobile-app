...
| `RequestFailure.refusedParameter(name)` | true for `parameters_validate_issue` naming that query parameter - the request was never considered, so asking again without it is a different question rather than a retry |
