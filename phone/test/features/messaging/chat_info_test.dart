...
void main() {
  ...
  group('Conversation list, the muted mark', () {
    ...
    Future<void> pumpTile(WidgetTester tester, Chat chat) async {
      // A row with a last message names its sender, and that asks the contact
      // repository; nobody answers unless it is stubbed.
        ...
        harness.wrap(
          BlocProvider<UnreadCountCubit>.value(
            ...
            child: PresenceViewParams(
              ...
              child: Scaffold(
                body: ChatConversationsTile(conversation: chat, lastMessage: lastMessage, userId: 'user-1'),
              ),
            ),
          ),
        ),
    ...
    testWidgets('sits in the same slot whatever the row is', (tester) async {
      // It used to ride inside the group row's name, next to the member
      // count, while every other row put it by the time - so the eye had to
      // look for it in two places down one list.
      double markToTime(WidgetTester tester) {
        final mark = tester.getTopRight(find.byIcon(Icons.notifications_off_outlined)).dx;
        return tester.getTopLeft(find.text(lastMessage.createdAt.timeOrDate)).dx - mark;
      }

      harness.withChatMute(7, const NotificationMute(muted: true));
      await pumpTile(tester, groupChat(id: 7, name: 'Team'));
      final onGroup = markToTime(tester);

      harness.withChatMute(6, const NotificationMute(muted: true));
      await pumpTile(tester, dialogChat(id: 6));

      expect(markToTime(tester), onGroup);
    });
