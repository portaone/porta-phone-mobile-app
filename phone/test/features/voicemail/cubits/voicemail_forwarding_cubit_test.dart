...
void main() {
  ...
  group('what the backend answered', () {
    Future<VoicemailForwardOutcome?> send(Object error) async {
      when(() => repository.forwardVoicemail(any(), toUserId: any(named: 'toUserId')))
          .thenAnswer((_) => Future.error(error));
      cubit.start(message);
      await cubit.sendTo(colleague());
      return cubit.state.report?.outcome;
    }

    test('a recording too big to pass on', () async {
      final outcome = await send(
        api.VoicemailForwardAttachmentTooLargeException(url: Uri(), requestId: 'r', statusCode: 413),
      );
    ...
    test('a colleague who can hold no more', () async {
      final outcome = await send(
        api.VoicemailForwardLimitReachedException(url: Uri(), requestId: 'r', statusCode: 422),
      );
    ...
    test('a backend that does not forward at all', () async {
      final outcome = await send(
    ...
    test('anything else, including a colleague who is no longer there', () async {
      // Past the three above the difference does not change what the person
      // can do about it.
      expect(
        await send(api.VoicemailForwardRecipientNotFoundException(url: Uri(), requestId: 'r', statusCode: 404)),
        VoicemailForwardOutcome.failed,
      );
    });
  });
