...
LocalCall _tracked({String callId = _kCallId}) {
  return LocalCall(callId: callId, status: CallProcessingStatus.incomingFromOffer, hasOffer: true);
}

/// A call a push registered whose offer never came as a live event.
LocalCall _awaitingOffer({
  String callId = _kCallId,
  CallProcessingStatus status = CallProcessingStatus.incomingFromPush,
}) {
  return LocalCall(callId: callId, status: status, hasOffer: false);
}
...
void main() {
  ...
  group('empty handshake', () {
    test('returns empty list when lines is empty', () async {
      final actions = await processor.process(lines: [], guestLine: null, localCalls: const []);
      expect(actions, isEmpty);
    });

    test('returns empty list when all lines are null', () async {
      final actions = await processor.process(lines: [null, null], guestLine: null, localCalls: const []);
      expect(actions, isEmpty);
    });
  });
  ...
  group('unanswered incoming call', () {
    test('returns HandleIncomingCallAction for single IncomingCallEvent', () async {
      ...
      final actions = await processor.process(lines: [line], guestLine: null, localCalls: const []);
    ...
    test('skips HandleIncomingCallAction when callId already in activeCallIds', () async {
      ...
      final actions = await processor.process(lines: [line], guestLine: null, localCalls: [_tracked()]);
  ...
  group('guestLine', () {
    test('processes guestLine the same as regular lines', () async {
      ...
      final actions = await processor.process(lines: [], guestLine: guestLine, localCalls: const []);
  ...
  group('offer delivery to a push-registered call', () {
    ...
    test('a call waiting for its offer gets the offer from the log', () async {
      ...
      final actions = await processor.process(lines: [line], guestLine: null, localCalls: [_awaitingOffer()]);
    ...
    for (final status in [
      ...
      test('a call answered before the handshake ($status) gets the offer too', () async {
        ...
        final actions = await processor.process(
          lines: [line],
          guestLine: null,
          localCalls: [_awaitingOffer(status: status)],
        );
    ...
    test('offer delivery comes after the per-line actions', () async {
      ...
      final actions = await processor.process(lines: [waiting, other], guestLine: null, localCalls: [_awaitingOffer()]);
    ...
    test('a plan that tears the session down carries no offer', () async {
      ...
      final actions = await processor.process(lines: [waiting, dying], guestLine: null, localCalls: [_awaitingOffer()]);
  ...
  group('conference block', () {
    ...
    test('no room reported, nothing to hang up', () async {
      final actions = await processor.process(lines: [], guestLine: null, localCalls: const [], conference: null);
      expect(actions, isEmpty);
    });
  });
