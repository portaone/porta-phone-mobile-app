...
void main() {
  ...
  test('the offer is answered, the legs grouped and taken off hold', () async {
    final h = _harness();
    addTearDown(h.close);
    ...
    // The server un-held the leg as it joined, with no event: the flag and
    // the OS follow the list.
    expect(h.bloc.state.retrieveActiveCall('b')!.held, isFalse);
    expect(h.callkeep.held, [(callId: 'b', onHold: false)]);
  });
  ...
  test('a call added to a live room is quiet from the ack and listed by the server', () async {
    ...
    expect(h.callkeep.held, contains((callId: 'c', onHold: false)));
