...
void main() {
  ...
  group('reading the mute pair off a payload', () {
    test('a conversation without the fields reads as nothing known, not as unmuted', () {
      // A core without the functionality sends the conversation bare. That is
      // not "not muted" - storing it as such would invent a row per
      // conversation on every sync - it is the absence of an answer.
      expect(NotificationMutePhxMapper.fromMapOrNull({'id': 42, 'name': 'engineering'}), null);
    });

    test('a conversation with the fields reads them like a reply does', () {
      final mute = NotificationMutePhxMapper.fromMapOrNull({
        'id': 42,
        'notifications_muted': true,
        'notifications_muted_until': null,
      });

      expect(mute, const NotificationMute(muted: true));
    });
    ...
    test('a null flag reads as not muted rather than throwing', () {
      // A failed cast here would surface inside the personal-topic stream and
      // restart the whole sync; a payload this odd degrades to "not muted".
      expect(NotificationMutePhxMapper.fromMap({'notifications_muted': null}), NotificationMute.none);
      expect(NotificationMutePhxMapper.fromMapOrNull({'id': 42, 'notifications_muted': null}), null);
    });
