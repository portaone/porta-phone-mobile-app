...
- (void)reportEndCall:(NSString *)uuidString
  ...
  // Ended from our side: it no longer keeps the screen awake, and a report completion that
  // arrives late for it finds it gone from the own-call set and leaves it out.
...
- (void)callObserver:(CXCallObserver *)callObserver callChanged:(CXCall *)call {
  ...
  if (call.hasEnded) {
    [self setVideo:NO forCallUUID:call.UUID];
  }
...
- (void)syncCallWaitingTone:(CXCallObserver *)callObserver {
  ...
  for (CXCall *call in callObserver.calls) {
    ...
    if (call.hasEnded) {
      ...
      [_videoCallUuids removeObject:call.UUID];
      [self refreshIdleTimer];
      continue;
    }
  ...
  [_delegateFlutterApi performEndCall:action.callUUID.UUIDString
                           completion:^(NSNumber *fulfill, FlutterError *error) {
                             if (error != nil || [fulfill boolValue] != YES) {
                               ...
                               [action fulfill];
                               [self setVideo:NO forCallUUID:action.callUUID];
                             }
                           }];
}
...
/// Whether [uuid] is one of this plugin's calls that is still live: reported by us, not ended
/// by us and not reset away, and not ended as far as the call observer knows. A call the
