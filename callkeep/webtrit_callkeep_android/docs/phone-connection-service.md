...
A call is made active whenever it crosses the boundary in either direction while Telecom holds
it. On the way out, because a hold answered silently during membership would otherwise leave it
held for Telecom and speaking for the application; on the way in, because a member Telecom left
holding would be a participant nobody can hear. Telecom's sequencing then holds whichever call
it must, and outside a group that hold reaches the application like any other.
